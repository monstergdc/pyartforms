...
# poly test - nn * flux
def demo_poly_test():
    files=['test-src1.png', 'test-src2.jpg']
    ...
    for nn in [3, 64]:
        for flux in [0, 5, 40, 100]:
            ...
            demo_looper(params, files=files, modes=['poly'])
...
def demo_brush_test():
    ...
    for nn in [5, 60, 100]:
        for bs in [5, 15, 40, 100]:
            ...
            demo_looper(params, files=files, modes=['brush'])
...
def demo_lines_test():
    ...
    params = {'w': w, 'h': h, 'bk': (0, 0, 0), 'coef': 0.9, 'scale': 1.0, 'rnd': True, 'rmin': 78, 'rmax': 98}
    # todo: more params
    demo_looper(params, files=files, modes=['lines'])
